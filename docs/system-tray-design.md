# ma-browser System Tray Daemon — 详细设计
...
把 ma-browser daemon 从「CLI 黑盒」演进为 **「Windows 系统级 AI 浏览器代理服务」** — 可见、可控、可自愈。
...
ma-browser 的全部 UI 分布在 **5 个层级**，每层有严格的职责边界。设计时优先问"这功能应该在哪一层"，再问"怎么做"。
...
| 🎯 托盘 ToolTip | `ma-browser · 已连接 · :19826` |
| 📋 弹窗 Body | 端口行恒显，一键复制 |
| 🖥 控制面板 Overview | 顶部端口卡片（可点击修改） |
| 💻 CLI | `ma-browser status` 打印两个端口 |
| 📝 stdout 启动横幅 | 启动时打印 |
...
ma-browser 中存在 **三类不同来源** 的"动作记录"，先前版本只分 2 类（把 ② ③ 混在一起）。
...
虽然 Microsoft 官方建议 Mica 用于持久窗口、Acrylic 用于瞬态弹层，但 ma-browser 是工具型应用，**统一 Acrylic 优先于遵循官方默认**。
...
| 3 | **CLI 入口去留** | 保留 `ma-browser daemon start` 等 CLI 命令 | 托盘是 superset 而非替代；CI/SSH/headless 场景继续可用；packages/cli 不动 |
